...
class MyRefCountBase {
 ...
 public:
  ...
  void Incref() noexcept { m_use++; }

  void Incwref() noexcept { m_weak++; }
  ...
  [[nodiscard]] long Use_Count() const noexcept { return m_use; }
};
...
class MyRefCount : public MyRefCountBase {
 ...
 private:
  void Destroy() noexcept override { delete m_ptr; }
...
class MySharedPtr final : public MyBaseSmartPtr<T> {
 public:
  ...
  void swap(MySharedPtr<T> &x) noexcept { this->Swap(x); }
  ...
  friend bool operator==(const MySharedPtr<T> &lhs, const MySharedPtr<T> &rhs) { return lhs.get() == rhs.get(); }

  friend bool operator!=(const MySharedPtr<T> &lhs, const MySharedPtr<T> &rhs) {
    return !(lhs == rhs);  // NOLINT
  }

  friend bool operator==(const MySharedPtr<T> &lhs, const T *rhs) { return lhs.get() == rhs; }

  friend bool operator!=(const MySharedPtr<T> &lhs, const T *rhs) {
    return !(lhs == rhs);  // NOLINT
  }

  friend void swap(MySharedPtr<T> &x, MySharedPtr<T> &y) noexcept { x.swap(y); }
};
...
class MyWeakPtr final : public MyBaseSmartPtr<T> {
 public:
  ...
  friend void swap(MyWeakPtr<T> &x, MyWeakPtr<T> &y) noexcept { x.swap(y); }

  MySharedPtr<T> lock() const noexcept;

  void reset() noexcept;

  [[nodiscard]] long use_count() const noexcept { return MyBaseSmartPtr<T>::Use_Count(); }
};
