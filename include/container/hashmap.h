...
class HashMap {
 ...
 private:
  ...
  struct node {
     ...
     */
    explicit node(const value_type &value = value_type(), node *next = nullptr) : value(value), next(next) {}
  };
...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap<K, M, H> &m) : _size{0}, _hash_function(m._hash_function) {
...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(HashMap<K, M, H> &&m) noexcept
...
template <typename K, typename M, typename H>
HashMap<K, M, H> &HashMap<K, M, H>::operator=(HashMap<K, M, H> &&m) noexcept {
  if (this == &m) {
    return *this;
  }
  clear();
