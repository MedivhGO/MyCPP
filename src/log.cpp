...
// get global logger
CLogger *get_logger() { return get_log_context()->logger; }
...
void start_file_logger(const char *log_file_path, bool append_mode, int max_rotation, size_t rotate_size, bool async,
                       millisecond_t sleep_time) {
...
void *CLogger::operator new(size_t size) { return stblog_aligned_alloc(alignof(CLogger), size); }

void CLogger::operator delete(void *ptr) { stblog_aligned_free(ptr); }
...
void *CLogHandler::operator new(size_t size) { return stblog_aligned_alloc(alignof(CLogHandler), size); }

void CLogHandler::operator delete(void *ptr) { stblog_aligned_free(ptr); }

CLogHandler::CLogHandler() : m_logger(nullptr), m_filter(nullptr), m_formatter(nullptr), m_closed(false) {
...
CLogFile::CLogFile(const char *filepath, bool append, int rotate_count, size_t rotate_size)
    : m_hfile(nullptr), m_curfile(filepath), m_cur_size(0), m_rotate_size(rotate_size), m_rotate_count(rotate_count) {
...
CCustomLog::CCustomLog() : m_str(nullptr, 0) {}
...
void CCustomLog::handle_error(const LogData *log) { printf("Fail to write log.\n"); }

CLogString::CLogString(size_t init_size) : CCustomLog(), m_buf(nullptr), m_capacity(init_size) {
    if (init_size < 1)
        init_size = 1;
