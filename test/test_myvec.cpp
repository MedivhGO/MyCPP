...
size_t DestructorType::destruction_count = 0;
...
TEST_F(MyVecTest, DISABLED_Init) {
  EXPECT_EQ(v.Size(), 0);
  EXPECT_EQ(v.Capacity(), 0);
}

TEST_F(MyVecTest, DISABLED_PushBack) {
...
TEST_F(MyVecTest, DISABLED_Insert) {
...
TEST_F(MyVecTest, DISABLED_Remove) {
...
TEST_F(MyVecTest, DISABLED_Clear) {
  MyVector<DestructorType> destructor;

  const size_t expected = 100;
  destructor.Reserve(expected);
  for (size_t i = 0; i < expected; ++i) {
    destructor.PushBack(DestructorType());
  }

  for (size_t i = 0; i < expected; ++i) {
    destructor.EmplaceBack(DestructorType());
  }
  ...
  EXPECT_EQ(DestructorType::destruction_count, expected * 2);
}

TEST_F(MyVecTest, DISABLED_Grow) {
...
TEST_F(MyVecTest, DISABLED_Alignment) {
...
TEST_F(MyVecTest, DISABLED_CopyConstructionAndDestruction) {
...
TEST_F(MyVecTest, DISABLED_DestroyElements) {
  MyVector<DestructorType> destructor;
  ...
  destructor.DestroyElements(0, insertions >> 1);

  for (size_t i = 0; i < insertions >> 1; ++i) {
    EXPECT_TRUE(destructor[i].destroyed);
  }
  for (size_t i = (insertions >> 1) + 1; i < destructor.Size(); ++i) {
    EXPECT_FALSE(destructor[i].destroyed);
  }
}

TEST_F(MyVecTest, DISABLED_Equality) {
