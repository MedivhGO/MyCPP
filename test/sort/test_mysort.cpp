...
TEST(MySort, test_heapsort) {
  ...
  auto run_stdsort = [&]() { std::sort(sorted_dataset.begin(), sorted_dataset.end()); };
  auto run_mysort = [&]() { HeapSort(test_dataset, test_dataset.size()); };
  run_stdsort();
  run_mysort();
...
TEST(MySort, test_quicksort) {
  ...
  std::vector<int> sorted_dataset = test_dataset;
  auto run_stdsort = [&]() { std::sort(sorted_dataset.begin(), sorted_dataset.end()); };
  auto run_mysort = [&]() { QuickSort(test_dataset, 0, test_dataset.size()); };
  run_stdsort();
  run_mysort();
  EXPECT_EQ(sorted_dataset, test_dataset);
}
...
TEST(MySort, test_quicksortmot) {
  ...
  std::vector<int> sorted_dataset = test_dataset;
  auto run_stdsort = [&]() { std::sort(sorted_dataset.begin(), sorted_dataset.end()); };
  auto run_mysort = [&]() { QuickSortMOT(test_dataset, 0, test_dataset.size()); };
  run_stdsort();
  run_mysort();
  EXPECT_EQ(sorted_dataset, test_dataset);
}
