...
TEST(MySmartPtrTest, TEST22) {
  ...
  EXPECT_FALSE(ptr);  // NOLINT(bugprone-use-after-move, clang-analyzer-cplusplus.Move): moved-from state is the point
                      // of this test
}
...
TEST(MySmartPtrTest, TEST27) {
  ...
  std::cout << sptrA.use_count() << '\n';  // 1 sptrA
  std::cout << sptrB.use_count() << '\n';  // 1 sptrB
